...
//
//-----------------------------------------------------------
class TribesTVInputPage extends ut2k3guIPage;
...
function bool MyOnDraw (Canvas canvas)
{
	...
	if ((pc != none) && (pc.myhud != none)) {
		pc.myhud.DrawTypingPrompt (Canvas, "TribesTVSAY " $ TypedStr);
	}
	else {
		Log ("TribesTV: Current player has no hud, closing chat");
...
function bool MyOnKeyEvent(out byte Key, out byte Action, float delta)
{
	...
	if (Key == 0x1b) {	//escape
		...
		if (TypedStr != "") {
			class'TribesTVReplication'.default.ChatString = TypedStr;
		}
		Controller.CloseMenu ();
