...
function ModifyCharacters()
{
         ...
         foreach AllActors(class'Gameplay.GameInfo', C)
			...
			//C.PlayerControllerClassName = VERSION_NAME $ ".x2PlayerCharacterController";
			//C.Default.PlayerControllerClassName = VERSION_NAME $ ".x2PlayerCharacterController";
               }
}
	    ...
	    if (EnablePod == True)
	      {
			...
			{
			vehiclePad.vehicleClass = class'x2_v2b.x2Pod';
			}
              }
	     ...
	     if (EnableTank == True)
	      {
			...
			{
			vehiclePad.vehicleClass = class'x2_v2b.x2Tank';
			}
              }
...
function Actor ReplaceActor(Actor Other)
{
	...
	{
		Other.Destroy();
		return ReplaceWith(Other, VERSION_NAME $ ".x2Character");
	} */
