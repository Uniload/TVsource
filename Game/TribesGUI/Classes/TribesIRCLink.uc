class TribesIRCLink extends IpDrv.BufferedTCPLink;
...
var TribesIRCSystem SystemPage;
...
function Connect(TribesIRCSystem InSystemPage, string InServer, string InNickName, string InUserIdent, string InFullName, string InDefaultChannel)
{
	local int i;

    log("TribesIRCLink Connect:"@InServer@InNickName@InUserIdent@InFullName@InDefaultChannel,'IRC');
...
function Resolved( IpAddr Addr )
{
	...
	Log( "Tribes TribesIRCLink: Server is "$ServerAddress$":"$ServerIpAddr.Port,'IRC' );
...
{
    log("TribesIRCLink: JoinChannel: "$Channel,'IRC');
	if ( Channel == "" )
		Channel = SystemPage.GetCurrentChannelName();
...
function PartChannel(string Channel)
{
	...
	SystemPage.SetCurrentChannel(SystemPage.CurChannel - 1);
...
defaultproperties
{
	...
	VersionString="Tribes IRC Client version "
