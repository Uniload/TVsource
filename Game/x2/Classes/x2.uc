...
function ModifyCharacters()
{
         ...
         foreach AllActors(class'Gameplay.GameInfo', C)
            if (C != None)
               {
			C.Default.DefaultPlayerClassName = "x2.x2Character";
			C.DefaultPlayerClassName = "x2.x2Character";
			C.PlayerControllerClassName = "x2.x2PlayerCharacterController";
			C.Default.PlayerControllerClassName = "x2.x2PlayerCharacterController";
               }
}
...
function Actor ReplaceActor(Actor Other)
{
	...
	{
		Other.Destroy();
		return ReplaceWith(Other, "x2.x2Character");
	}
