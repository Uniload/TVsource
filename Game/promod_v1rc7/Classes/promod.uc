...
function ModifyInvStationAccessControl()
{
	local Gameplay.InventoryStation InvStat;
	local Gameplay.PlayerCharacter C;
	local PlayerReplicationInfo P;
		
	forEach AllActors(class'Gameplay.InventoryStation', InvStat)
		if(P.bHasFlag == true)
			InvStat.InventoryStationAccessClass = Class'promodInventoryStationAccess';
//			InvStat.CanBeUsedBy(C)
} 
...
//{
//	promod_v1rc7.promodHeadShotStatThreshold.OnHeadShot(Pawn instigatedBy, Pawn target, class<DamageType> damageType, float amount);
//}
...
{
//	local Gameplay.SniperRifle weapon;
...
//		if(weapon != None)
//			if(ShouldModifyWeaponDevice(weapon))
//				weapon.noAmmo = SniperPlayerThreshold;

	local Gameplay.Equippable weapon;
	
	ForEach AllActors(Class'Equippable', weapon)
		if (weapon != None)
			if(ShouldModifyWeaponDevice(weapon))
				weapon.bHidden = SniperPlayerThreshold;
...
function bool ShouldModifyWeaponDevice(Equippable weapon)
{
	local int i;

	for(i = 0; i < WeaponInclusionList.Length; i++)
		if(weapon.IsA(WeaponInclusionList[i].Name))
			return true;
        return false;
}
...
//			c.team().combatRoleData[2].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7.DefaultArmorHeavy", class'class')).default.AllowedWeapons;
