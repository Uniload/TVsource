...
const VERSION_NAME = "promod_v1rc6";
...
//					if(CapperCanReinvo == false && TournamentOn())
//						{
...
// Disable inventory when carrying a flag in tournament mode
//ModifyInventoryStationAccess()
//{
//	if (PlayerReplicationInfo.HasFlag != None && TournamentOn)
//		{
//		promod.promodInventoryStationAccess.usedByProcessing(Character user); 
//		}
//}
...
//{
//	promod_v1rc6.promodHeadShotStatThreshold.OnHeadShot(Pawn instigatedBy, Pawn target, class<DamageType> damageType, float amount);
//}
	    ...
	    if(EnablePod)
	      {
			...
			vehiclePad.vehicleClass = class'promod_v1rc6.promodPod';
	      }
            else if(!EnablePod)
              {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehiclePod')
            ...
            if(EnableRover)
	      {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleBuggy')
				...
				if(EnableRoverGun)
					{
					vehiclePad.vehicleClass = class'promod_v1rc6.promodDefaultBuggy';
					} else {
					vehiclePad.vehicleClass = class'promod_v1rc6.promodBuggy';
					}
				}
		      else if(!EnableRover)
				{
            ...
            if(EnableTank)
	      {
			...
			vehiclePad.vehicleClass = class'promod_v1rc6.promodTank';
	      }
            else if(!EnableTank)
              {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleTank')
...
function ModifyStats()
{
	...
	if(M != None && BonusStatsOn && !TournamentOn() && CTFOn())
	{
		...
		// Head Shot
		M.projectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc6.promodSniperProjectileDamageType';
...
//				 SniperRifle HS
//		M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc6.promodSniperProjectileDamageType';
//		++statCount;
		
                // E-Blade
		M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc6.promodBladeProjectileDamageType';
...
//		WeaponSniperRifle(Other).playerDamageStatClass = Class'Gameplay.ExtendedStat'
	...
	{
		Other.Destroy();
		return ReplaceWith(Other, "promod_v1rc6.promodWeaponEnergyBlade");
		return Super.ReplaceActor(Other);
	}
		...
		if(TankUsesMortarProjectile == true)
			{
                ...
                WeaponVehicleTank(Other).projectileClass = Class'promod_v1rc6.promodTankRound';
...
function string MutateSpawnCombatRoleClass(Character c)
{
	...
	c.team().combatRoleData[2].role.default.armorClass.default.health = HOHealth; //was 195
		...
		c.team().combatRoleData[0].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc6.promodArmorLight", class'class')).default.AllowedWeapons;
		c.team().combatRoleData[1].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc6.promodArmorMedium", class'class')).default.AllowedWeapons;
		c.team().combatRoleData[2].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc6.promodArmorHeavy", class'class')).default.AllowedWeapons;
