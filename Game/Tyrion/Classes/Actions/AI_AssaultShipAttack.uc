//=====================================================================
// AI_AssaultShipAttack
//=====================================================================

class AI_AssaultShipAttack extends AI_VehicleAction
	editinlinenew;
...
const CLOSING_RANGE = 4000.0f;			// how close to get to target before starting fancy combat maneuvers
const ATTACK_SPEED = 1500.0f;			// attack speed
const HEIGHT_ABOVE_TARGET = 1500.0f;	// how high do you want to be above target when you attack?
const DIAGONAL_DISTANCE = 5000;
...
Begin:
	...
	while ( class'Pawn'.static.checkAlive( v ) && class'Pawn'.static.checkAlive( target ) && (followGoal == None || !followGoal.wasNotAchieved() ))
	{
		...
		// 2. move to a point behind target - call doLocalMove *without* target flag set
		//   (since assault ship just passes overhead and drops bombs)
		if ( class'Pawn'.static.checkAlive( v ) && class'Pawn'.static.checkAlive( target ) && (followGoal == None || !followGoal.wasNotAchieved() ))
		{
			destination = target.Location - v.Location;
			destination.Z = 0;
			...
			if ( followFunction == None || followFunction.validDestination( destination ) )
			{
				waitForGoal( (new class'AI_VehicleLocalAttackGoal'( driverResource(), achievingGoal.priority, 
									destination, 750, ATTACK_SPEED, None )).postGoal( self ), true );
			}
		}
	}
