...
event deleteRemovedActions()
{
	...
	{
		//log( "deleteRemovedActions: deleting" @ removedActions[i].name );
		removedActions[i].Release();	// Comment out this line to disable AI action deletion
	}
...
{
	local AI_Action bestAction;				// return value
	local AI_Action action;
	local array<AI_Action> matchingActions;	// sorted list of actions that match 'goal'
	local int i, j;

	// create a sorted list of actions that achieve 'goal'
	for ( i = 0; i < abilities.length; i++ )
	{
		action = AI_Action(abilities[i]);

		//log( "@@@" @ pawn().name @ action.name @ action.satisfiesGoal.name @ goal.class @ action.selectionHeuristic( goal ) );
		//if ( ClassIsChildOf( goal.class, action.satisfiesGoal ) )
		//	log( "@@@" @ action.name @ "sat's" @ goal.name @ usedByAction @ usedByAction.achievingGoal.priorityFn() @ goal.priorityFn() );

		if ( ClassIsChildOf( goal.class, action.satisfiesGoal ) &&
			( usedByAction == None ||											// resource not in use
			  usedByAction.achievingGoal.priorityFn() < goal.priorityFn() ||	// resource doing something less important
			  multipleActionsCheck( action ) ||									// the resource can handle multiple actions
			  doesParentHaveResource( goal.parentAction )))						// the parent of the goal is already using the resource
		{
			action.heuristicValue = action.selectionHeuristic( goal );

			if ( action.heuristicValue > 0 )
			{
				// insert 'action' into 'matchingActions', sorted by heuristicValue
				for( j = 0; j < matchingActions.Length; ++j )
				{
					if ( matchingActions[j].heuristicValue < action.heuristicValue )
						break;
				}
				matchingActions.insert(j, 1);
				matchingActions[j] = action;
			}
		}
	}

	// find the action with the highest heuristicValue
	for ( i = 0; i < matchingActions.length; i++ )
	{
		action = AI_Action( class'Engine.Tyrion_Setup'.static.shallowCopy( matchingActions[i] )).myAddRef();	// make a shallow copy
		// log( "ChooseAction: Created new action" @ action.name @ "from" @ matchingActions[i] ); 

		if ( setActionParameters( goal, action ) )	// returns false if parameters named in this goal couldn't be set
		{
			bestAction = action;
			break;
		}
...
event resourceCheck( AI_Goal goal, optional AI_Action bestAction )
{
	...
	//log( "RESOURCE_CHECK: goal:" @ goal.name @ "action:" @ bestAction.name @ "usedBy:" @ usedByAction );
...
{
	return Rook(pawn());	// returns the same value as pawn() by default
}
#endif
...
function removeGoal( AI_Goal goal )
{
	...
	//    resources may have been freed up (handled in handleGoalFailure/handleGoalSuccess)
	...
	//    (handled in interruptAction which calls handleGoalFailure)
	// 5. A permanent inactive goal changes state
	//    (handled in activate/deactivate/handleGoalFailure/handleGoalSuccess)
	...
	for ( i = 0; i < goals.length; i++ )
		if ( goals[i] == goal )
		{
			...
			goal.bDeleted = true;
			goal.parentAction = None;
			//log( "->" @ goal.name @ "in" @ pawn().name @ "RELEASED" );
			if (goal.Release() > 0)
			{
				//assert(!goal.Outer.IsA('Actor'));
				goal.NullReferences();
			}
			break;
		}
...
function displayTyrionDebug()
{
	...
	if ( class'Pawn'.static.checkAlive( debugPawn ) && goals.length > 0 )
	{
		...
		for ( i = 0; i < goals.length; i++ )
		{
			...
			if ( goal.bInactive )
			{
				color = class'Canvas'.static.MakeColor(0,255,200);
			...
			debugPawn.AddDebugMessage( "  " $ goal.goalDebuggingString() @ "(" $ goal.priorityFn() $ ")" @ inactiveString, color );
			if ( goal.achievingAction != None )
