...
var NS_Action nsChild;								// a Tyrion action can start a Cersei child (just one for now)
...
latent function succeed()
{
	...
	achievingGoal.notifyCompletion(true);
...
event instantSucceed()
{
	...
	// we need a goto state if the instantSucceed is called from state code 
	// (rather than the normal latent succeed function)
	gotostate('');
}
...
event instantFail( ACT_ErrorCodes errorCode, optional bool bRemoveGoal )
{
	...
	achievingGoal.notifyCompletion(false);
...
// The goals list will not be cleaned up by the stack cleanup function
// because is currently doesn't handle array parameters so the function
// could cause crashes when saving the game.
...
function removeAction()
{
	...
	// remove any Cersei children
	if ( nsChild != None )
	{
		nsChild.removeAction();
		nsChild = None;
	}
