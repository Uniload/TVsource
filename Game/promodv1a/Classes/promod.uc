...
var config float cgVelocity;
var config float cgMinSpread;
var config float cgMaxSpread;
var config float ePackBoost;
var config float ePackBoostDuration;
     ...
     foreach AllActors(class'Gameplay.VehicleSpawnPoint', vehiclePad)
	    ...
	    if(EnablePod)
            ...
            if(EnableRover)
	      {
			...
			vehiclePad.vehicleClass = class'promodv1a.promodBuggy';
	      }
            else if(!EnableRover)
              {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleBuggy')
...
function ModifyStats()
{
	...
	if(M != None)
	{
                for(i = 0; i < M.extendedProjectileDamageStats.Length; ++i)
		{
			...
			{
				M.ProjectileDamageStats[i].headShotStatClass = None;
			}
                }
		...
		M.extendedProjectileDamageStats.Insert(statCount, 4);
                ...
                M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promodv1a.promodBladeProjectileDamageType';
...
function Actor ReplaceActor(Actor Other)
{
        if(Other.IsA('WeaponChaingun'))
	{
		...
		WeaponChaingun(Other).projectileVelocity = cgVelocity;
		WeaponChaingun(Other).minSpread = cgMinSpread;
		WeaponChaingun(Other).maxSpread = cgMaxSpread;
                WeaponChaingun(Other).heatPeriod = 1000;
	...
	{
                EnergyPack(Other).boostImpulsePerSecond = ePackBoost; //was 75000.000000
	        ...
	        EnergyPack(Other).durationSeconds = ePackBoostDuration; //was 1.000000
...
function string MutateSpawnCombatRoleClass(Character c)
{
	...
	c.team().combatRoleData[2].role.default.armorClass.default.health = HOHealth;
        ...
        c.team().combatRoleData[0].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promodv1a.promodArmorLight", class'class')).default.AllowedWeapons;
	c.team().combatRoleData[1].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promodv1a.promodArmorMedium", class'class')).default.AllowedWeapons;
	c.team().combatRoleData[2].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promodv1a.promodArmorHeavy", class'class')).default.AllowedWeapons;
...
cgVelocity=32000.000000
cgMinSpread=0.400000
cgMaxSpread=2.000000
ePackBoost=75000.000000
ePackBoostDuration=1.000000
}