...
const VERSION_NAME = "promod_v1rc4";
	    ...
	    if(EnablePod)
	      {
			...
			vehiclePad.vehicleClass = class'promod_v1rc4.promodPod';
	      }
            else if(!EnablePod)
              {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehiclePod')
            ...
            if(EnableRover)
	      {
			...
			vehiclePad.vehicleClass = class'promod_v1rc4.promodBuggy';
	      }
            else if(!EnableRover)
              {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleBuggy')
            ...
            if(EnableTank)
	      {
			...
			vehiclePad.vehicleClass = class'promod_v1rc4.promodTank';
	      }
            else if(!EnableTank)
              {
              if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleTank')
...
function ModifyStats()
{
	...
	if(M != None && BonusStatsOn && !TournamentOn() && CTFOn())
	{
		...
		M.extendedProjectileDamageStats.Insert(statCount, 7); // we have 7 new stats
                ...
                // E-Blade
		M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc4.promodBladeProjectileDamageType';
...
function string MutateSpawnCombatRoleClass(Character c)
{
	...
	c.team().combatRoleData[2].role.default.armorClass.default.health = HOHealth; //was 195
        ...
        c.team().combatRoleData[0].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc4.promodArmorLight", class'class')).default.AllowedWeapons;
	c.team().combatRoleData[1].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc4.promodArmorMedium", class'class')).default.AllowedWeapons;
	c.team().combatRoleData[2].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc4.promodArmorHeavy", class'class')).default.AllowedWeapons;
